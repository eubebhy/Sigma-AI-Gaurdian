...
"""
File path: browser_tab.py
Input contract:
- open_tab(url): nhan vao chuoi bat dau bang http:// hoac https://
Output contract:
- tra ve True neu da goi duoc browser hop le
- tra ve False neu khong co browser hop le hoac that bai, khong nem loi ra ngoai
Operating principle:
- scan browser pho bien co executable/process ro rang
- uu tien browser dang chay co CLI hop le
- neu khong co thi thu default browser cua OS
- neu van khong duoc thi fallback theo do pho bien
- browser diem cao nhat se duoc dung de mo tab
"""
...
def _run_open_command(command: list[str]) -> bool:
    try:
        # Keep launch detached and silent because open_tab must fail silently.
        if os.name == "nt":
...
def open_tab(url: str) -> bool:
    # Accept only http(s) URLs because the module is scoped to browser tabs.
    ...
    for browser in _pick_browser(require_running=False):
        if browser["executable"] and _run_open_command([browser["executable"], url]):
            return True
    return webbrowser.open(url, new=2)  # new=2 means open in a new tab, if possible
